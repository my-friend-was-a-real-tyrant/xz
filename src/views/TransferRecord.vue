    ...
    <div class="transfer-box">
      ...
      <div class="content">
        <ul v-if="navIndex == 0">
            ...
            <div class="titles" style="padding:10px 0 0 0">星座名称：<span style="color:#fff">{{item.goods_name}}</span></div>
            <div class="titles" style="padding:10px 0 0 0">星座价值：<span style="color:#fff">{{item.order_money}}</span></div>
            <div class="titles" style="padding:10px 0 0 0">智能合约收益：<span style="color:#fff">{{item.profit_interval}}</span></div>
            <div class="titles" style="padding:10px 0 0 0">到期转让时间：<span style="color:#fff">{{item.pipei_time}}</span></div>
            <div class="titles" style="padding:10px 0 0 0">状态：
              <span style="color:#fff">待转让</span>
              </div>
          </div>
        </ul>
        <ul  v-if="navIndex == 1">
          <div class="reservation">
            <li v-for="(item, index) in beenList" :key="index">
              ...
              <div class="right"><a @click="goTa(item.id)"><div class="bbuu">查看</div></a></div>
            </li>
          </div>
        </ul>
        <ul  v-if="navIndex == 2">
        ...
        <ul  v-if="navIndex == 3">
          <div class="reservation">
            <li  v-for="(item, index) in brokenList" :key="index">
              ...
              <div class="right"><a @click="goSsa(item.id)"><div class="bbuu">申诉</div></a></div>
            </li>
          </div>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    goTa(orderid){
      ...
      // 跳转详情
      this.$router.replace('/ta')
    },
    goSsa(orderid){
      localStorage.setItem('aaorderid', orderid)
      this.$router.replace('/ssa')
    },
    switchTab(index) {
      this.navIndex = index;
      this.status = index;
    ...
    getAdopt(){
      ...
      this.$post('/api?m=Api&c=Order&a=order_chu_list', data).then( (res)=>{
          if(res.status === 1){
            console.log(res)
              switch(this.status){
                case 0:
